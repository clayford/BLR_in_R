...
Let's convert a few probabilities to odds and log odds. The function `fractions()` from the MASS package returns fractions, which is usually how odds are expressed. (We use `as.character()` to print the fraction bar.) Notice odds are always positive, and that log odds can be positive or negative.
...
It's hard to tell if the polynomial is necessary based on the summary output. One proper way to assess this is using the `Anova()` function from the car package. This uses a likelihood ratio test to compare the model without the age polynomial to one with the age polynomial.
...
```{r}
ggeffect(model = m_poly, terms = "age") |> 
  plot(log.y = FALSE)
```
...
rbind(before = coef(m3), after = coef(m3a))
```

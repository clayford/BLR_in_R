...
Before we talk about the model, let's explain the stuff at the bottom:
...
```{r}
cbind("age 50" = pred1, "age 40" = pred2, "Diff" = pred1 - pred2)
```
...
For example, z-value for sexMale is 0.3360447. To get p-value, find probability z is greater than 0.3360447 and less than -0.3360447. Since Normal distribution is symmetric, just need to find one and multiply by 2. 

```{r}
# prob z > 0.3360447
pnorm(0.3360447, lower.tail = FALSE) * 2
```
...
Adding interactions and/or non-linear terms make model interpretation extremely difficult. One way to make sense of your model is to use _effect plots_. 
...
Below we show how to do this using the {ggeffects} package. 

First we'll visualize `m_nle` using `ggpredict()`. The first argument is our model object, the second argument specifies which variable, or model term, we wish to visualize. This produces predictions at various ages while holding sex and uncons at "Female" and "No", respectively. These are the reference levels for each variable.

```{r}
ggpredict(m_nle, terms = "age")
```

We can pipe this result into `plot()` to get a basic effect plot. The light gray ribbon is a 95% confidence band. Notice the ribbon gets larger (more uncertain) when we have fewer observations, which we see in the extremes of the data. It appears the effect of age doesn't change until after about age 55, and then it begins to increase. 
...
If we want to condition on sex = "Male" and uncons = "Yes", we can set them using the condition argument. They need to be specified using a vector. Notice how large the confidence ribbon gets since we have relatively few people were unconscious when admitted to the icu.
...
Notice the plots above are on the probability scale, which is usually desirable. But the fitted lines will usually curve reflecting the non-linear transformation from log odds to probabilities. 

We can make the plots on the log-odds scale but label them with probabilities by setting `log.y = TRUE`. This will generate plotted lines that reflect the coefficients in the model. For example, in m_nle, age is modeled as a non-linear effect. By setting `log.y = TRUE` we can see the fitted non-linear effect. Notice the curve is slight and could easily fit within the confidence ribbon as a straight line. (Recall the non-linear coefficient was not significant.) Also notice the uneven spacings on the y-axis.
...
Next, we visualize m_int which models an interaction between age and uncons. This allows the effect of uncons to depend on age, and vice versa. (Recall the interaction was small and the p-value was about 0.11.) Using `ggpredict()`, we specify the variables of interest as a vector. The first variable will be plotted on the x-axis. The syntax `age[all]` tells `ggpredict()` to use all age values when making predictions.

```{r}
ggpredict(m_int, terms = c("age[all]", "uncons")) |>
  plot()
```

There is a great deal of uncertainty about the effect of age when a patient is unconscious. That's because of the 200 patients, only 15 were admitted unconscious. And no one under the age of 40 was unconscious when admitted to the ICU. By setting `log.y = TRUE` we can plot on log odds scale but label the axis with probabilities. In addition we can separate the plots using `facet=TRUE`.

```{r}
ggpredict(m_int, terms = c("age[all]", "uncons")) |>
  plot(log.y = TRUE, facet = TRUE)
```

If we reverse the order of variables in the `terms` argument we get uncons on the x-axis and age set to 3 distinct values (mean of age and +/- 1 standard deviation.

```{r}
ggpredict(m_int, terms = c("uncons", "age")) |>
  plot()
```

```{r}
mean(icu$age)
mean(icu$age) + c(-1,1)*sd(icu$age)
```

Standard deviation is really only appropriate if your data is symmetric and age is certainly not symmetric.

```{r}
hist(icu$age)
```

Fortunately we can set our own values for age when we create the effect plot. Below we get the 25th, 50th, and 75th percentiles of age, save as a vector named "v", and then use that object in the call to `ggpredict()`.

```{r}
v <- quantile(icu$age, probs = c(0.25, 0.50, 0.75))
ggpredict(m_int, terms = c("uncons", "age[v]")) |>
  plot()
```

Of course that didn't really change the plot all that much. 
...
Here are the exact values of the labeled points. A Cook's D bigger than 1 is the usual red flag. A Hat Value is considered large if it exceeds (2 or 3)(k/n) where k is the number of model coefficients and n is number of observations. Here that comes out to (2 or 3)(7/200) or about (0.07 - 0.11). Residuals larger than 2.5 may be of interest.
...
By dropping subject 155, the coefficient of the uncons main effect dropped from 13.13 to 2.46, and the age:uncons interaction reversed sign and saw an increase in standard error. This doesn't necessarily mean we should remove this subject from the analysis. It just means they're influencing the model and we should exercise caution and modesty in drawing conclusions.

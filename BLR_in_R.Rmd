...
What if we sampled 100 males and 100 females and compared the proportion with tattoos? Perhaps 0.18 of males have tattoos and 0.12 of females have tattoos. This suggests the probability of having a tattoo may be related to gender.
...
```{r}
p <- c(0.0001, 0.001, 0.01, 0.1, 0.3, 0.5, 0.7, 0.9, 0.99, 0.999, 0.9999)
...
We're not sure if being Male increases or decreases the chance of dying. The lower bound is negative and the upper bound is positive (though not by much). 
...
```{r}
m2 <- glm(died ~ age + admit + uncons, data = icu, family = binomial)
```
...
Let's fit a model where age and systolic interact.

```{r}
m3 <- glm(died ~ age + systolic + age:systolic, data = icu, family = binomial)
summary(m3)
```

The small p-value might suggest an interaction is present. But notice how small the coefficient is. Is this interaction important? How can we assess this?
...
We can pipe this result into `plot()` to get a basic effect plot. The colored ribbons are 95% confidence bands. It appears as systolic gets lower the effect of age on death increases.
...
```{r}
predict_response(m3, terms = c("age", "systolic[95, 130, 165]")) |>
  plot()
```
...
Let's use this plot with our `m4` model that we fit in the previous code along.
...
Below are the exact values of the labeled points. A Cook's D bigger than 1 is the usual red flag. A Hat Value is considered large if it exceeds (2 or 3)(k/n) where k is the number of model coefficients and n is number of observations. Here that comes out to (2 or 3)(7/200) or about (0.07 - 0.11). Residuals larger than 2.5 may be of interest.

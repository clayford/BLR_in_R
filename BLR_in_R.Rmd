...
_We are responsible for proposing and assessing the model!_ For example, the "sex" variable may or may not help our model. We have to decide to retain it or drop it. Subject matter expertise is just as important as statistical expertise.
...
The model is in the Estimate column of the coefficients section. Here's the model expressed in basic math notation, with values rounded to two decimal places:
...
- small changes in probability can result in "big" changes in log odds. log odds change rapidly when p is close to 0 or 1.
...
No intro logistic regression would be complete without a plot of probability versus log odds. 
...
```{r}
# can also use drop1(m, test = "Chisq")
Anova(m)
```
...
It's hard to tell if the spline is necessary based on the summary output. One proper way to assess this is using the `Anova()` function from the car package. This uses a likelihood ratio test to compare the model without the age spline to one with the age spline.
...
```{r}
ggeffect(model = m_nle, terms = "age") |> 
  plot(log.y = TRUE)
```
...
```{r}
ggeffect(model = m_int, terms = c("age", "uncons")) |>
  plot(facets = TRUE)
```
...
```{r}
m3 <- glm(died ~ age + cancer + admit + uncons + systolic + age:systolic,
...
- Subject 84: model predicts death at 94%, but subject lived
...
- Subject 285: model predicts death at 5%, but subject died
...
```{r}
m3a <- glm(died ~ age + cancer + admit + uncons + systolic + age:systolic,
          data = icu, family = binomial, 
...
Below is more material that had to be cut for time.
...
## Assessing predictive accuracy via calibration
...
The `rms` package provides the `validate` function for this purpose. However it only works for models fit using `rms` functions. Instead of `glm()`, we need to use `lrm()`. We also need to specify `x = TRUE, y = TRUE` so the original data is stored in the model object. To see a summary of the fit, we simply need to print the model object. Notice it returms more output than a model fit with `glm()`, including several discrimination indices.

```{r}
library(rms)
m_rms <- lrm(died ~ age + uncons, 
